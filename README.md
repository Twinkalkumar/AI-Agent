...
## Adding new capabilities (e.g. GitHub access)

`instructions.yaml` can only turn tools on/off per task — it can't create a
brand-new capability by itself. Adding something genuinely new (like GitHub
access) takes two steps:

1. **Write the real Python code** for the capability — see `tools/github_tools.py`
   for a working example (list repos, list files, read a file, write/commit
   a file), then register each function + its schema in `TOOL_REGISTRY`
   inside `agent.py`.
2. **Reference the tool names in a task** in `instructions.yaml` — this part
   really is just adding a block, once step 1 exists:
   ```yaml
   github_assistant:
     description: "Browse, read, and modify files in your GitHub repos"
     system_prompt: >
       You are a GitHub assistant. ... always confirm before writing.
     tools: [github_list_repos, github_list_files, github_read_file, github_write_file]
   ```
...
1. Create a personal access token at
   ...
   **Use a fine-grained token scoped to only the specific repo(s)** you want
   the agent to touch, with `Contents: Read and write` permission — avoid
   giving it access to every repo you own.
2. Add to your `.env`:
   ```
   GITHUB_TOKEN=ghp_...
   GITHUB_DEFAULT_OWNER=your-github-username   # optional shorthand
   ```
3. Rebuild and restart:
   ```bash
   docker compose build agent
   docker compose up -d agent
   ```
4. Select the `github_assistant` task in the UI dropdown (or `/task github_assistant`
   in the terminal).

**Important safety note:** `github_write_file` commits directly to the repo
and branch you give it — there's no built-in confirmation dialog, only the
system prompt instructing the model to check with you first before writing.
A model can still misfire. Keep the token scoped as narrowly as possible,
and consider testing on a throwaway repo before pointing this at anything
you care about.
